...
function userRegistration(filePath) {
    ...
    transporter.sendMail(mailOptions(email, "Підтвердження реєстрації", `Код підтвердження ${token}`), function (err, data) {
        if (err) {
...
//                 const token = otp.authenticator.generate(salt)
//                 console.log("Відправляємо лист підтвердження")
//                 transporter.sendMail(mailOptions(email, "Скидання паролю", `Код підтвердження ${token}`), function (err, data) {
//                     if (err) {
//                         console.log("Error " + err);
//                         return
//                     } else {
